...
enum TIPO_GIOCO tipoGioco=GIOCO_TENNIS;
				...
				case PLAY_ENDING:
					...
					switch(tipoGioco) {
						...
						case GIOCO_PELOTA:
							...
							if(tipoGioco<GIOCO_SQUASH) {		// in alcune foto è al contrario, tratteggio il bordo e intera la rete...
...
//			bInFront=GetPrivateProfileInt(APPNAME,"SempreInPrimoPiano",0,INIFile);
			...
			tipoGioco=GetPrivateProfileInt(APPNAME,"TipoGioco",1,INIFile);
			...
			timeBeginPeriod(1);		// è lento.. e non fa nulla..
			hTimer=SetTimer(hWnd,1,20,NULL);
